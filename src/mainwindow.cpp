...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent)
{
	...
	codeViewFont.setFamily("Courier");
	codeViewFont.setFixedPitch(true);
	codeViewFont.setPointSize(10);
	m_ui.codeView->setFont(codeViewFont);
	QFontMetrics codeViewFontMetrics(codeViewFont);
	m_ui.codeView->setTabStopWidth(codeViewFontMetrics.width(' ')*4);
	m_ui.codeView->setReadOnly(true);

	highlighter = new Highlighter(m_ui.codeView->document());
	...
	connect(m_ui.commandLine, SIGNAL(returnPressed()), SLOT(onCommandLineReturnPressed()));
	m_tagScanner.init();
...
void MainWindow::ICore_onStopped(ICore::StopReason reason, QString path, int lineNo)
{
	...
	if(!path.isEmpty()) {
		m_ui.codeView->setHighlightLine(m_currentLine);
		//open(path);
	} else
		m_ui.codeView->setHighlightLine(-1);
...
void MainWindow::onThreadWidgetSelectionChanged( )
{
...
void MainWindow::onWatchWidgetCurrentItemChanged( QTreeWidgetItem * current, int column)
{
	QTreeWidget *varWidget = m_ui.varWidget;
	Core &core = Core::getInstance();
...
void MainWindow::open(QString filename)
{
	...
	m_ui.codeView->setPlainText(file.readAll());
	m_filename = filename;
...
void MainWindow::ICore_onCurrentFrameChanged(int frameIdx)
{
	...
	if(frameIdx < m_stackFrameList.size()) {
		...
		if (m_currentFile != entry.m_sourcePath && !m_currentFile.isEmpty()) {
...
void MainWindow::onFuncListItemActivated(int index)
{
	...
	if(lineIdx < 0)
		lineIdx = 0;
	m_ui.codeView->scrollToBlock(lineIdx);
}

void MainWindow::onCommandLineReturnPressed()
{
	QString line = m_ui.commandLine->text();
	QByteArray byteArray = line.toUtf8();
	Core &core = Core::getInstance();
	core.sendRawCommand(byteArray.constData());
}
